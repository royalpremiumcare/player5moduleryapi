...
- [x] **ota-decision:** OTA kararı VERİLDİ → Capgo Cloud (manuel entegrasyon). Plugin `@capgo/capacitor-updater@7.50.2` kuruldu; config + `notifyAppReady` (rollback) + Android native bağlandı; ilk kanal `internal`. Kalan: internal'da yayın/rollback testi → `production` promote (komutlar OPERATIONS.md §5).
