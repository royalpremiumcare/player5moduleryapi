...
def send_payout_success_email(
    ...
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #10b981;">✓ Ödemeniz Yolda</h2>
        ...
        <p style="color: #9ca3af; font-size: 12px;">PLANN — İşletme Yönetim Platformu</p>
    </div>
    """
    return _send_email(to_email, merchant_name, subject, html)
...
def send_payout_failed_email(
    ...
    html = f"""
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #ef4444;">✗ Ödeme Gönderilemedi</h2>
        <p>Merhaba {merchant_name},</p>
        <p>
            <strong>{amount_display}</strong> tutarındaki ödemeniz gönderilemedi.
            Tutar cüzdanınıza iade edildi.
        </p>
        {f'<p style="color: #6b7280;">Hata: {error_message}</p>' if error_message else ''}
...
def send_dispute_alert_email(
    ...
    subject = f"⚠️ İtiraz (Dispute) bildirimi — {amount_display}"
    html = f"""
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #f59e0b;">⚠️ Ödeme İtirazı</h2>
        <p>Merhaba {merchant_name},</p>
        <p>
        ...
        <p style="color: #6b7280; font-size: 14px;">Dispute ID: {dispute_id}</p>
...
def send_reconciliation_alert(
    ...
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 800px; margin: 0 auto;">
        <h2 style="color: #ef4444;">🚨 Günlük Mutabakat Uyuşmazlığı Tespit Edildi</h2>
        ...
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 24px 0;">
        <p style="color: #9ca3af; font-size: 12px;">PLANN Financial Engine — Otomatik Mutabakat Sistemi</p>
    </div>
    """
    return _send_email(SUPERADMIN_ALERT_EMAIL, "PLANN SuperAdmin", subject, html)
...
def send_rate_spike_alert(
    ...
    subject = f"{arrow} GBP/TRY Kur Sıçraması — %{change_pct}"
    html = f"""
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #f59e0b;">{arrow} Kur Sıçraması Tespit Edildi</h2>
        ...
        <p>TRY tier limitleri otomatik olarak güncellenmiştir (Currency Shield).</p>
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 24px 0;">
        <p style="color: #9ca3af; font-size: 12px;">PLANN Financial Engine — Currency Shield</p>
    </div>
    """
    return _send_email(SUPERADMIN_ALERT_EMAIL, "PLANN SuperAdmin", subject, html)
...
def send_kyc_pending_alert(
    ...
    html = f"""
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #f59e0b;">🚨 KYC Onayı Bekleyen İşletme</h2>
        ...
        <p>SuperAdmin panelinin <strong>Finansal</strong> sekmesinden KYC onayını verebilirsiniz.</p>
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 24px 0;">
        <p style="color: #9ca3af; font-size: 12px;">PLANN Financial Engine — KYC Alerts</p>
    </div>
    """
    return _send_email(SUPERADMIN_ALERT_EMAIL, "PLANN SuperAdmin", subject, html)
...
def send_aml_review_alert(
    ...
    html = f"""
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #f59e0b;">🔍 AML İnceleme Gerekli</h2>
        ...
        <p>SuperAdmin panelinden onaylayabilir veya reddedebilirsiniz.</p>
        <hr style="border: none; border-top: 1px solid #e5e7eb; margin: 24px 0;">
        <p style="color: #9ca3af; font-size: 12px;">PLANN Financial Engine — Compliance</p>
    </div>
    """
    return _send_email(SUPERADMIN_ALERT_EMAIL, "PLANN SuperAdmin", subject, html)
